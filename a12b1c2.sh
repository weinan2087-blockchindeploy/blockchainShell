...
########################################
###  [ orga，orgb，orgc，12，1，2 ]  ###
########################################

export_a_ip=127.0.0.11
export_a_group_id=1
...
# 1. check param and depen depend on catalog
if [ ! -n "$1" ] ;then
...
if [ ! -n "$3" ] ;then
    echo error ! third parameter cannot be null 
    exit 1
fi
if [ ! -d "console" ]; then
...
# 8.console config 
# 8.1 copy console to org catalog
cd ../
cp -r console ./$1 && cp -r console ./$2 && cp -r console ./$3
# 8.2 generator orgs sdk and copy it to console catalog
...
# 9. generator nodes install package
cd ../
